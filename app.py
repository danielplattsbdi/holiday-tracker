...
# ===== Cancellation backend config (you gave me these) =====
...
CANCEL_TOKEN = "adfouehrgounvroung8168evs"   # must match Apps Script SHARED_SECRET
...
def load_team_df() -> pd.DataFrame:
    ...
    out["Office"] = raw[office_col].fillna("Unassigned").astype(str).map(lambda s: s.strip().title()) if office_col else "Unassigned"
    ...
    return (out.dropna(subset=["Member"]).drop_duplicates("Member").sort_values("Member").reset_index(drop=True))

# --------- Requests (merge Office from Team; form does NOT need Office) ---------
@st.cache_data(ttl=60)
...
# If you prefer to limit to selected year, uncomment:
...
if not options:
    ...
    if st.button("Cancel selected request", type="primary", use_container_width=True, key="cancel_button"):
        if not CANCELLATION_ENDPOINT or not CANCEL_TOKEN:
            st.warning("To actually cancel, make sure CANCELLATION_ENDPOINT and CANCEL_TOKEN are set.")
        else:
            try:
